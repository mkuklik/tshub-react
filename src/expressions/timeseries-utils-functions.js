...
export function start(ts) {
  if (!(ts instanceof TS)) throw Error('start: not a timeseries');
  return (ts.data && ts.data.length > 0) ? ts.data[0][0] : undefined;
}

export function start2(ts, ts2) {
  if (!(ts instanceof TS)) throw Error('start2: not a timeseries');
...
export function end(ts) {
  ...
  return (ts.data && ts.data.length > 0) ? ts.data[ts.data.length - 1][0] : undefined;
}

export function end2(ts, ts2) {
  if (!(ts instanceof TS)) throw Error('end2: not a timeseries');
  return moment.max(moment.max(ts.indices()), moment.max(ts2.indices()));
}

export function trim(ts) {
  /* trim NaN from both sides */
  if (!(ts instanceof TS)) throw Error('trim: not a timeseries');
  let ileft = 0;
  let iright;
  ts.data.forEach((x, i) => {
    if ((ileft === i) && isNaN(x[1])) ileft = i + 1;
    if (!isNaN(x[1])) iright = i + 1;
  });
  const data = ts.data.slice(ileft, iright);
  // const data = ts.data.slice(ileft, iright).map(x => [x[0].clone(), x[1]]); // ? should I clone;
...
export function drop(ts) {
  ...
  const valid = Array.apply(true, Array(ts.data.length));
  ts.data.forEach((x, i) => {
    if (isNaN(x[1])) valid[i] = false;
  });
  const data = ts.data.filter((x, i) => valid[i]);
  // const data = ts.data.slice(ileft, iright).map(x => [x[0].clone(), x[1]]); // ? should I clone;
  return new TS(data, ts.freq, ts.fparams, ts.dtype, ts.dparams, ts.units, ts.options);
}
...
export function seq(start, end, freq) {
  return Array.from(moment.range(start, end).by(_duration(freq)));
}

// select subset of observations from
export function window(ts, start, end) {
  // TODO check if start adn end was parsed correctly
  if (!(ts instanceof TS)) throw Error('window: not a timeseries');
  ...
  if (isMoment(start)) {
    ...
    _start = moment.utc(start);
    if (_start === undefined) throw Error('window: start is invalid');
  } else if (start instanceof Date) {
    _start = moment.utc(start);
  } else if (start === undefined) {
    _start = start;
  } else {
    throw Error('window: start is invalid');
  }

  if (_start !== undefined) {
    data = data.filter((x) => !x[0].isBefore(_start));
  }
  ...
  if (isMoment(start)) {
    ...
    _end = moment.utc(end);
    if (_end === undefined) throw Error('window: end is invalid');
  } else if (end instanceof Date) {
    _end = moment.utc(end);
  } else if (end === undefined) {
    _end = end;
  } else {
    throw Error('window: end is invalid');
  }

  if (_end !== undefined) {
    data = data.filter((x) => x[0].isSameOrBefore(_end));
  }
...
export function merge(l, r, join) { // TODO
  ...
  */
  ...
  if (r.freq !== l.freq) throw TypeError('Frequency mismatch');

  const inx = seq(start2(l, r), end2(l, r), r.freq);
  const data = inx.map((x) => {
    const a = l.value(x);
    const b = r.value(x);
    if (a === NaN || b === NaN) return [x, NaN];
    return [x, a + b];
  });
...
export function lag_A(x, n) {
  if (n === undefined) n = 1;
  const data = x.data.map((y) => [y[0].clone().subtract(n, 'years'), y[1]]);
  return new TS(data, x.freq, x.fparams, x.dtype, x.dparams, x.units, x.options);
}

export function lag(x, n) {
  if (!(x instanceof TS)) throw Error('lag: not a timeseries');
...
export function sign(ts) {
  /* sign of timeseries values */
  if (!(ts instanceof TS)) throw Error('start2: not a timeseries');
  const data = ts.data.map((y) => [y[0].clone(), y[1] > 0]);
  return new TS(data, ts.freq, ts.fparams, 'bool', ts.dparams, {}, ts.options);
}

const dateFormats = {
  Q: ['YYYY[Q]Q', 'YYYY-Q'],
  M: ['YYYY[M]MM', 'YYYY[M]M', 'YYYY-MM', 'YYYY-M'],
  W: ['YYYY[W]W', 'YYYY-W', 'YYYY-WW'],
  D: ['YYYY-M-D', 'YYYY-MM-DD'],
  A: ['YYYY'],
};

export function parseToDate(str, freq) {
  const fmts = dateFormats[freq];
  if (R.isNil(fmts)) {
  ...
  if (R.isNil(output)) throw Error(`parsing ${str} date to frequency ${freq} failed`);
  return output;
}

// export function countna(ts) {
//   /* number of NA, NaN in the series */

// }

// export function expavg(ts, alpha) {
//   /* exponencial average */
// }

// export function fillin(ts, method) {
//   /* fill in NA/NaN values with method
//   zero
//   linear
//   next
//   previous
...
// export function first(ts) {
//   /* first valid value */
// }

// export function last(ts) {
//   /* last valid value */
// }

// export function indexmax(ts) {
//   /* index of maximum value */
// }

// export function indexmin(ts) {
