...
const gimage 		= require('google-image-chart').charts;

const timeseries = function (data, options) {
  /*
		Data Format:
		[
			[Date Object, value],
...
timeseries.prototype.clone = function () {
  const buffer = _.map(this.data, (point) => [
    point[0],
    point[1] * 1,
  ]);
  return buffer;
...
timeseries.prototype.toArray = function () {
  return _.map(this.data, (datapoint) => datapoint[1]);
};
...
timeseries.prototype.ema = function (options) {
  ...
  const m	= 2 / (options.period + 1);	// Multiplier
...
timeseries.prototype.pixelize = function (options) {
  ...
  const min 	= this.min();
  const max 	= this.max();
  const tile	= (max - min) / options.grid;

  this.buffer	= _.map(this.data, (datapoint) => {
...
timeseries.prototype.supports = function (options) {
  ...
  _.each(this.data, (datapoint) => {
    const val = Math.round(datapoint[1] / tile) * tile;
    if (!prices[val]) {
      prices[val] = 0;
    }
    prices[val]++;
  ...
  ordered	= _.filter(ordered, (support) => support.count >= options.threshold);
  if (options.stats) {
    return 	ordered;
  }

  return _.map(ordered, (support) => support.price);
};
    ...
    for (i = 2; i < l; i++) {
      if (options.forecast == i) {
        ...
        for (j = 2; j < 720; j++) {
          ...
          var ratio	= d1 / d2;
          const mse		= (ratio - buffer_forecast[i][2]) * (ratio - buffer_forecast[i][2]);
...
timeseries.prototype.regression_forecast = function (options) {
  ...
  const sample 		= buffer.slice(options.start - 1 - options.sample, options.start);

  // The current data to process is only a sample of the real data.
  this.data		= sample;
  // Get the AR coeffs
  const coeffs 		= this[options.method]({ degree: options.degree });
...
timeseries.prototype.regression_forecast_mse = function (options) {
  ...
  const backup 		= _.map(options.data, (item) => [
    item[0],
    item[1] * 1,
  ]);
  const buffer 		= _.map(options.data, (item) => [
    item[0],
    item[1] * 1,
  ]);
  ...
  for (i = options.sample; i < l - 1; i++) {
    ...
    const knownValue 	= buffer[i + 1][1] * 1;
    buffer[i + 1][1]	= 0;
    for (j = 0; j < coeffs.length; j++) {
      if (options.method == 'ARMaxEntropy') {
...
timeseries.prototype.ARMaxEntropy = function (options) {
  ...
  const pef 	= this.fill(0, length);
  const per 	= this.fill(0, length);
  let ar 		= this.fill([], options.degree + 1);
  ar			= _.map(ar, (d1) => scope.fill(0, options.degree + 1));
  const h 		= this.fill(0, length);
...
timeseries.prototype.ARLeastSquare = function (options) {
  ...
  let i; let j; let k; let hj; let hi;
  const coefficients = [];
...
timeseries.prototype.SolveLE = function (mat, vec, n) {
  ...
  let i; let j; let k; let maxi;
  let vswap 		= [];
  let mswap 		= [];
  let hvec 		= [];
  let max; let h; let pivot; let q;
...
timeseries.prototype.regression_analysis = function (options) {
  ...
  let i;
  let j;
  const E 		= this.data.length; // total number of input spaces
  let N 		= 0;
  let N1 		= 0;
  let N2 		= 0;
  let SUM 	= 0.0;
  ...
  const EM 	= 1 + (2 * N1 * N2) / (N1 + N2); // Mean "Mu"
  const SD1 = [2 * N1 * N2 * (2 * N1 * N2 - N1 - N2)];
  const SD2 = Math.pow((N1 + N2), 2);
  const SD3 = N1 + N2 - 1;
  const SD4 = SD1 / (SD2 * SD3); // Standard deviation "Sigma"
  const SD 	= Math.sqrt(SD4);
  // calculating P value MStyle
  const z1 	= (R - EM) / SD;
  const z2 	= Math.abs(z1);
  var z 	= z2;

  /* Thanks to Jan de Leeuw for the following function */
  var t 	= (z > 0) ? z : (-z);
  const P1 	= Math.pow((1 + t * (0.049867347 + t * (0.0211410061 + t * (0.0032776263 + t * (0.0000380036 + t * (0.0000488906 + t * (0.000005383))))))), -16);
  const p 	= 1 - P1 / 2;
  var t 	= 1 - ((z > 0) ? p : 1 - p); // this is P-value

  // rounding the value
  const t1 	= Math.round(100000 * t);
  const t2 	= t1 / 100000; // this is P-value too
  // form.PV.value = t2;
  ...
  // AUTO CORRELATION
  let DWNN = 0;
  let DWND = (cval[0] * cval[0]);
  for (i = 1; i < cval.length; i++) {
    DWNN += (cval[i] - cval[i - 1]) * (cval[i] - cval[i - 1]);
    DWND += (cval[i] * cval[i]);
  }
  let DW = DWNN / DWND;
  ...
  const Q01 	= 2 - 4.6527 / (Math.sqrt(N + 2));
  const Q05 	= 2 - 3.2897 / (Math.sqrt(N + 2));
...
adapter.fromDB = function (data, options) {
  ...
  return _.map(data, (datapoint) => [new Date(datapoint[options.date]).getTime(), datapoint[options.value]]);
};
adapter.fromArray = function (data) {
  return _.map(data, (datapoint) => [new Date(), datapoint]);
};
