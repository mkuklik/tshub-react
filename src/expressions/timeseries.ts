...
import type { IDType, IFreq } from '../viewer/types/Tcommon';
import { either, isEmpty, isNil, mergeDeepRight, map, clone } from 'ramda';

const eitherEmptyOrNil = either(isEmpty, isNil);
...
export class TS {
  data: [moment.Moment, number][];
  freq: IFreq;
  fparas?: object;  // Type this more specifically if known
  dtype: IDType;
  dparams?: object; // Type this more specifically if known
  units?: string; // units can be undefined now
  options: TSOptions;


  constructor(data: [moment.Moment, number][], freq: string, fparams: any, dtype: string, dparams: any, units?: string, options: TSOptions = {}) {
    /*
      Data Format:
      [
        [Date Object, value],
    ...
    */
    this.options = mergeDeepRight({}, options);
    this.data = data;
    this.dtype = dtype;
    this.dparams = dparams;
    this.freq = freq;
    this.fparas = fparams;
    this.units = units; //  Now optional
  }

  clone = (): TS => new TS(clone(this.data), this.freq, this.fparas, this.dtype, this.dparams, this.units, this.options);



  values = (): number[] => this.data.map(x => x[1]);

  indices = (): moment.Moment[] => this.data.map(x => x[0]);

  nobs = (): number => this.data.length;

  get = (inx: moment.Moment): [moment.Moment, number] | undefined => this.data.find(x => x[0].isSame(inx));

  value = (inx: moment.Moment): number => {
    const tmp = this.get(inx);
    if (eitherEmptyOrNil(tmp)) return Number.NaN;
    return tmp[1];
  };

  index = (inx: moment.Moment): moment.Moment | undefined => {
    const tmp = this.get(inx);
    return tmp ? tmp[0] : undefined; // Simplified conditional return
  };

  equals = (x: any): boolean => {  // Explicitly typed parameter 'x'
    if (!(x instanceof TS)) return false;
    return x.data === this.data;
  };


  toArray = (): number[] => map(datapoint => datapoint[1], this.data);

  print = (format?: string): [string, number][] => this.data.map(x => [x[0].format(format || 'MMM Do YY'), x[1]]);

  isSame = (x: any): boolean => {  // Explicitly typed parameter 'x'
    if (!(x instanceof TS)) return false;
    if (this.data.length !== x.data.length) return false;
    const index = this.data.every((z, i) => z[0].isSame(x.data[i][0]));
    const value = this.data.every((z, i) => z[1] === x.data[i][1]);
    return index && value;
  };
}
...
// converts Chronos data to timeseries object
export function fromChronos(chronosTimeseries: ChronosTimeseries, chronosData: [moment.Moment, number][], options?: any): TS {
  const data = chronosData;
  return new TS(data, chronosTimeseries.freq, chronosTimeseries.fparams, chronosTimeseries.dtype, chronosTimeseries.dparams, undefined, options);
}


export function create_tsD(): TS {
  const data: [moment.Moment, number][] = [
...
// const eitherEmptyOrNil = R.either(R.isEmpty, R.isNil);

// export class TS {
//   constructor(data, freq, fparams, dtype, dparams, units, options) {
//     /*
//       Data Format:
//       [
//         [Date Object, value],
...
//   clone = function () {
//     return TS(R.clone(this.data), this.freq, this.fparams, this.dtype, this.dparams, this.options);
//   }

//   values = () => this.data.map((x) => x[1]);

//   indices = () => this.data.map((x) => x[0]);

//   nobs = () => this.data.length;

//   get = (inx) => this.data.find((x) => x[0].isSame(inx));

//   value = (inx) => {
//     const tmp = this.get(inx);
//     if (eitherEmptyOrNil(tmp)) return NaN;
//     return tmp[1];
//   }

//   index = (inx) => {
//     const tmp = this.get(inx);
//     if (eitherEmptyOrNil(tmp)) {
//       return tmp;
//     }
//     return tmp[0];
//   }
...
//   equals = (x) => {
//     if (!x instanceof TS) return false;
//     if (x.data === this.data) return true;
//   }

//   // Convert the data to a 1D array
//   toArray = function () {
//     return R.map((datapoint) => datapoint[1], this.data);
//   };

//   print = (format) => this.data.map((x) => [x[0].format(format || 'MMM Do YY'), x[1]])
...
// // var timeseries = function (data, freq, fparams, dtype, dparams, units, options) {
// // 	/*
// // 		Data Format:
// // 		[
// // 			[Date Object, value],
...
// // converts Chronos data to timeseries object
// export function fromChronos(chronosTimeseries, chronosData, options) {
//   const data = chronosData; // TODO
//   return TS(data, chronosTimeseries.freq, chronosTimeseries.fparams, chronosTimeseries.dtype, chronosTimeseries.dparams, options);
// }

// // // export function check_types(x) {}
...
// // // start of the period
// // export function start(ts) {
// //   if (!(ts instanceof TS)) throw TypeError("start works on time series only");
// //   return (ts.data) ? ts.data[0][0] : undefined;
// // }

// // export function start2(ts, ts2) {
// //   return moment.min(moment.min(ts.indices()), moment.min(ts2.indices()))
// // }

// // // end of the period
// // export function end(ts) {
// //   if (!(ts instanceof TS)) throw TypeError("start works on time series only");
// //   return (ts.data) ? ts.data[ts.data.length - 1][0] : undefined;
// // }

// // export function end2(ts, ts2) {
// //   return moment.max(moment.max(ts.indices()), moment.max(ts2.indices()))
// // }

// // // generate index for
// // export function seq(start, end, freq) {
// //   switch (freq) {
// //     case "D":
...
// //     case "Q":
// //       return Array.from(moment.range(start, end).by("quarter"));
// //     default:
// //       throw Error(`unsupported frequency, ${freq}`)
// //   }
// // }

// // // select subset of observations from
// // export function window(ts, start, end) {
...
// // export function merge(l, r, join) {
// //   /*
// //   join is either inner or outer
// //   */
// //   join = join || 'outer'
// //   if (!(l instanceof TS && r instanceof TS)) {
// //     throw Error("merge: one of the inputs is not a timeseries")
// //   }
...
// // function lag_D(x, n) {
// //   if (n === undefined) n = 1;
// //   const data = x.data.map(y => [y[0].clone().subtract(n, 'days'), y[1]]);
// //   return new TS(data, x.freq, x.fparams, x.dtype, x.dparams, x.units, x.options)
// // }

// // function lag_W(x, n) {
// //   if (n === undefined) n = 1;
// //   const data = x.data.map(y => [y[0].clone().subtract(n, 'weeks'), y[1]]);
// //   return new TS(data, x.freq, x.fparams, x.dtype, x.dparams, x.units, x.options)
// // }
// // function lag_M(x, n) {
// //   if (n === undefined) n = 1;
// //   const data = x.data.map(y => [y[0].clone().subtract(n, 'months'), y[1]]);
// //   return new TS(data, x.freq, x.fparams, x.dtype, x.dparams, x.units, x.options)
// // }
// // function lag_Q(x, n) {
// //   if (n === undefined) n = 1;
// //   const data = x.data.map(y => [y[0].clone().subtract(n, 'quarters'), y[1]]);
// //   return new TS(data, x.freq, x.fparams, x.dtype, x.dparams, x.units, x.options)
// // }

// // export function lag(x, n) {
// //   // value.value =
// //   // value.dtype =
// //   // value.freq =
// //   // value.units =
...
// //       return lag_Q(x, n);
// //     default:
// //       throw Error("unsupported frequency, " + x.freq);
// //   }
...
// //   if (base === undefined) throw "log: base is undefined";
// //   if (x instanceof TS) {
// //     // value.value =
// //     // value.dtype =
// //     // value.freq =
