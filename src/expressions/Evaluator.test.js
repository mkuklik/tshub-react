...
test('isSingleReference 1', () => {
  ...
  const check = evaluator.isSingleReference();
...
test('isSingleReference 3', () => {
  const evaluator = new Evaluator('$xyz({start: "2019-01-01"})');
  const check = evaluator.isSingleReference();
  expect(check).toBeTruthy();
});

test('isSingleReference 4', () => {
  const evaluator = new Evaluator('log($xyz)');
  const check = evaluator.isSingleReference();
  expect(!check).toBeTruthy();
});


test('isSingleReference 5', () => {
  const evaluator = new Evaluator('$xyz + 1');
  const check = evaluator.isSingleReference();
  expect(!check).toBeTruthy();
});

test('isSingleReference 4', () => {
  const evaluator = new Evaluator('1 + $xyz');
  const check = evaluator.isSingleReference();
  expect(!check).toBeTruthy();
});
...
test('evaluator extras', () => {
  ...
  const e = new Evaluator('$abc({start: "2019-01-01", realtime: Date("2020-01-01")})');
  const [value, err] = e.evaluate(data)
  expect(value).toStrictEqual(123);
});
...
test('parsing error handling', () => {
  ...
  const value = evaluator.evaluate(data);
  expect(value).toStrictEqual(123);
});
...
test('evaluator subtract gnp', () => {
  ...
  let x = sampleData.gnp;
  x.data = x.data.map(x => [moment.utc(x[0]), x[1]]);
  const expectedValue = new TS(x.data.map(x => [moment.utc(x[0]), 0.0]), x.freq, x.fparams, x.dtype, x.dparams, x.units);
  const data = {
    '_ref0': new TS(x.data, x.freq, x.fparams, x.dtype, x.dparams, x.units),
