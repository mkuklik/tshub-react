...
function* resolveExpr({ expr, realtime }: ISeries): IResolvedSeries {
  let resolvedSeries: IResolvedSeries = {
    expr,
    data: [],
    errors: [],
  ...
  // fetch observations
  const refsData: RefsData = yield fetchReference({ refs, realtime });

  const fetchedErrors = R.filter(
    (x) => !R.isNil(x),
    R.flatten(Object.values(refsData).map((x) => x.error))
  );
  ...
  const data = R.mapObjIndexed(
    (x) => new TS(x.data, x.freq, x.fparams, x.dtype, x.dparams),
    refsData
  );
  ...
  if (!(ts instanceof TS)) {
    ...
    resolvedSeries.errors = [
      ...resolvedSeries.errors,
      "Expression didn't evaluate to a timeseries",
    ];
    return resolvedSeries;
  }
