...
const createChildNodes = (collections, selectedCollectionID) => {
  ...
  if (collections.length > 0) {
    return collections.map(({ collId, name }) => ({
      ...
      label: <CollectionNodeLabel>{name}</CollectionNodeLabel>,
    }));
  }
...
const SpaceBrowserBase = (
  ...
  const handleRetryFetchCollectionsButtonClick = React.useCallback(
    (node) => () => {
      onRefetchSpaceCollections({
        ...node,
        childNodes: [],
  ...
  const handleDeleteSpaceCollectionDialogConfirm = React.useCallback(() => {
    ...
    if (selectedNode.type === "space") {
      ...
      console.error(
        "Unknown node.type in handleDeleteSpaceCollectionDialogConfirm, ",
        selectedNode.type
      );
    }
    toggleDeleteSpaceCollectionOverlay();
    ...
    () =>
      spaces.map(({ spaceId, name }) => {
        ...
        const spaceNode = {
          ...
          secondaryLabel: (
            <>
              {loadingSpaces[spaceId] && <SpaceLoadingIndicator size={20} />}
              {overSpaceId === spaceId && expandedSpaces[spaceId] && (
                ...
                >
                  <Icon
                    icon="plus"
                    onClick={() => toggleCreateCollectionOverlay(spaceId)}
                  />
                </Tooltip>
              )}
            </>
...
const SpaceBrowser = compose(Loadable)(SpaceBrowserBase);
