...
class DropdownSeriesSelector extends React.PureComponent {
  ...
  render() {
    const {
      allGraphSeriesDef, disabled, allSeries, selected, trim,
    } = this.props;
    ...
    if (selected.length === 1) {
      ...
      const name = (!isNil(allSeries[value].name))
        ...
        : null;
      const nameTrimed = (isNumber(trim) && !isNil(name)) ? name.slice(0, trim) : name;
      buttonText = (
        <div>
          <ColorIndicator color={path(['color'], find((x) => x.wsid === value, allGraphSeriesDef))} />
